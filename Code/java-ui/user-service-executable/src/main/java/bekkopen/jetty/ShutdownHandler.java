...
public class ShutdownHandler extends AbstractHandler {
    ...
    public void handle(String target, Request serverRequest, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        // System.out.println("ShutdownHandler - handle : " + target + " - " + serverRequest + " : " + request + " - " + response);
        String method = request.getMethod();
        String secretParam = request.getParameter("secret");
//        System.out.println("ShutdownHandler - handle " + target + " : " + method + " : " + secretParam + " - saved secret : "+ secret);
        if (!target.equals("/shutdown") || !request.getMethod().equals("POST") || !secret.equals(secretParam)) {
            // System.out.println(" - bail out : " + target + " - " + request.getMethod() + " - my secret : " + secret + " == " + request.getParameter("secret"));
            return;
        }
        System.out.println(" - try shut down!");
        try {
            ...
            System.err.println("Failed : " + ignored);
        }
        System.exit(0);
    }

    public static String shutdown(int port, String shutdownSecret) {
      System.out.println("SHUTDOWN URL : " + "http://127.0.0.1:" + port + "/shutdown?secret=" + shutdownSecret);
