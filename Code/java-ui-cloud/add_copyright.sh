...
if grep "miracle" "$AUTH" -q; then
cat - >> "$TEMP" <<END
//* (C) Copyright Miracle A/S, Denmark. 2015. All Rights Reserved.    *
END
fi
if grep "alexandra" "$AUTH" -q; then
cat - >> "$TEMP" <<END
//* (C) Copyright Alexandra Instituttet A/S, Denmark. 2015. All       *
//* Rights Reserved.                                                  *
END
fi
if grep "microsoft" "$AUTH" -q; then
cat - >> "$TEMP" <<END
//* (C) Copyright Microsoft Corp. 2015. All Rights Reserved.          *
END
fi
if grep "nsn" "$AUTH" -q; then
cat - >> "$TEMP" <<END
