...
public class ITInspector {
  ...
  public void inspectPresentationToken() throws Exception {
    ...
    Builder initResource =
        client.resource(baseUrl + "/inspector/init/soderhamn").accept(MediaType.TEXT_PLAIN);
    ...
    initVerifierAndUser(CryptoEngine.IDEMIX, CryptoEngine.IDEMIX);
...
//        + XmlUtils.toXml(of.createPresentationToken(presentationToken)));
    ...
    Builder inspectResource =
        client.resource(baseUrl + "/inspector/inspect?issuedValue=42").accept(MediaType.TEXT_PLAIN);
    System.out.println("Inspect!");

    String inspectionResult =
        inspectResource.post(String.class, of.createPresentationToken(presentationToken));
  ...
  public void OLDinspectPresentationToken() throws Exception {
    ...
    try {
      ...
      InputStream is = getClass().getResourceAsStream(xml_resource);
    ...
    Builder verifyResource =
        client.resource(baseUrl + "/inspect/1").type(MediaType.APPLICATION_XML)
...
//    File folder;
    ...
    if(UserHelper.getInstance().credentialManager.listCredentials().size()>0) {
      ...
      if (clientRunEngine == CryptoEngine.IDEMIX) {
        ...
        InputStream is =
            AbstractHelper.getInputStream("/eu/abc4trust/sampleXml/smartcard/sampleSecret.xml");
        ...
        InputStream is =
            AbstractHelper.getInputStream("/eu/abc4trust/sampleXml/patras/uprove-secret.xml");
        System.out.println("IS : " + is);
        secret = (Secret) XmlUtils.getObjectFromXML(is, true);
        ...
        for (int i = 0; i < scopes.length; i++) {
          ...
          try {
            ...
            PseudonymWithMetadata pseudo =
                UserHelper.getInstance().credentialManager.getPseudonym(pseudonymUID);
  ...
  //  presentationTokenResoruce += "presentationTokenWithInspection.jaxbobj";
    ...
    if(presentationTokenResoruce.endsWith("xml")) {
      String ptXml = InspectorHelper.loadObjectFromResource(presentationTokenResoruce);
//      System.out.println("XML : " + ptXml);
      ...
      presentationToken = InspectorHelper.loadObjectFromResource(presentationTokenResoruce);
    }
