  ...
  String[] presentationPolicyResources =
    ...
    public void initHelper(CryptoEngine cryptoEngine, CryptoEngine clientEngine, String userStorage) {
      try {
          ...
          if(cryptoEngine == CryptoEngine.BRIDGED) {
              issuerParamsResourceList = createIssuerParamsWithCryptoEngine(folder, issuerParamsResourceList_base, CryptoEngine.IDEMIX, CryptoEngine.UPROVE);
          } else {
              issuerParamsResourceList = createIssuerParamsWithCryptoEngine(folder, issuerParamsResourceList_base, cryptoEngine);
          }
          
          System.out.println("issuerParamsResourceList " + issuerParamsResourceList.length + " : " + issuerParamsResourceList);
          ...
          UserHelper.initInstance(cryptoEngine/* , systemParamsResource */,
                  issuerParamsResourceList, user_fileStoragePrefix, credSpecResourceList, inspectorPublicKeyResourceList);

          IssuerParameters exists =
                  UserHelper.getInstance().keyManager.getIssuerParameters(new URI(
    ...
    public void initIssuer(CryptoEngine cryptoEngine, CryptoEngine clientEngine) throws Exception {
      ...
      Builder initResource =
              client.resource(baseUrl + "/issue/init/" + cryptoEngine+"?clientEngine="+clientEngine).accept(MediaType.TEXT_PLAIN);
    ...
    protected void runIssuance(String serverMethod, String issuanceKey) throws Exception {
        runIssuance(serverMethod, issuanceKey, null);
    }
    protected void runIssuance(String serverMethod, String issuanceKey, String scope) throws Exception {
        ...
        System.out.println(" - initial message - client - created " + UserHelper.getPseudonymValue(user_im.im, scope));
	...
	protected void runVerification(CryptoEngine cryptoEngine, CryptoEngine clientEngine, String policy, boolean satisfiedExpected, String scope) throws Exception {
        ...
        System.out.println("PresentationToken pseudonym value for scope - " + scope + " : " + VerificationHelper.getPseudonymValue(presentationToken, scope));
