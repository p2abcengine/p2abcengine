...
public class UProveSerializer {
    ...
    public Node serialize(PseudonymComposite proof, URI nymAlias,
        ...
        uprovePseudonymElement = this.wrapper
                .createElement(U_PROVE_PSEUDONYM);
    ...
    public Element serialize(PresentationProofComposite proof, URI credAlias,
        ...
        proofElement
        .appendChild(this.createByteElement(TOKEN_ID,
                tokenId));
        proofElement
        .appendChild(this.createByteElement(MESSAGE_D,
                messageD));
        

        proofElement.appendChild(this.createArrayOfbase64BinaryElement(
                DISCLOSED_ATTRIBUTES, proof.getDisclosedAttributes()
                .getValue()));
        proofElement.appendChild(this.createArrayOfbase64BinaryElement(R_NAME,
                proof.getR().getValue()));
        proofElement.appendChild(this.createArrayOfbase64BinaryElement(
                TILDE_O, proof.getTildeO().getValue()));
        proofElement.appendChild(this.createArrayOfbase64BinaryElement(
                TILDE_VALUES,
                proof.getTildeValues().getValue()));
        ...
        proofElement.appendChild(this
                .createUProveTokenElement(compositeToken));
        proofElement.appendChild(this
                .createIssuerParametersCompositeElement(ipc));
    ...
    public Node createKeyLengthElement(int keyLength) {
        Element k = this.wrapper.createElementNS(SYSTEM_PARAMETERS_NS, KEY_LENGTH);
        k.setPrefix("uprove");
        k.setTextContent("" + keyLength);
        return k;
    }

    public Node createGroupOIDElement(String oid) {
      Element e = this.wrapper.createElementNS(SYSTEM_PARAMETERS_NS, GROUP_OID);
      e.setPrefix("uprove");
      e.setTextContent(oid);
      return e;
    }
    
    public Node createIssuerParametersCompositeElement(
            IssuerParametersComposite ipc) {
        Element e = this.wrapper.createElement(U_PROVE_CREDENTIAL_AND_PSEUDONYM);
        ...
        issuerParametersElement.appendChild(this
                .createArrayOfbase64BinaryElement(G_NAME, ipc
                        .getG().getValue()));
        issuerParametersElement.appendChild(this.createByteElement(GD,
                this.getValueAndCheckForNull(ipc.getGd())));
        issuerParametersElement.appendChild(this.createStringElement(GQ, ipc
                .getGq().getValue()));
        ...
        issuerParametersElement.appendChild(this.createStringElement(UID_H,
                ipc
                .getUidH().getValue()));
    ...
    private Node createUProveTokenElement(UProveTokenComposite compositeToken) {
        ...
        uproveTokenElement.appendChild(this.createByteElement(H,
                compositeToken.getH().getValue()));
        uproveTokenElement.appendChild(this.createByteElement(PI,
                compositeToken.getPI().getValue()));
        uproveTokenElement.appendChild(this.createByteElement(
                SIGMA_C_PRIME, compositeToken.getSigmaCPrime().getValue()));
        uproveTokenElement.appendChild(this.createByteElement(
                SIGMA_R_PRIME, compositeToken.getSigmaRPrime().getValue()));
        uproveTokenElement.appendChild(this.createByteElement(
                SIGMA_Z_PRIME, compositeToken.getSigmaZPrime().getValue()));
    ...
    private Node createIntElementArray(String name,
    ...
    public PresentationProofComposite deserializeToPresentationProofComposite(
        ...
        ArrayOfbase64Binary base64Binary = this.getBase64Binary(DISCLOSED_ATTRIBUTES, e);
        pc.setDisclosedAttributes(ofup.createPresentationProofCompositeDisclosedAttributes(base64Binary));
    ...
    private ArrayOfint getArrayOfint(Element e, String name) {
        ...
        for (int inx = 0; inx < nodes.getLength(); inx++) {
            ...
            if(n.getNodeType() != 1){
                if(n.getNodeType() == 3){
                    System.out.println("DEBUG: node type is of the type 'Text' - just whitespace ? [" + n.getTextContent().trim() + "]");
                } else {
                    System.out.println("WARN: node type is not of the type WC3Dom Element - node type # : " + n.getNodeType() + " : " + n);
                }
            	continue;
            }
    ...
    public UProveTokenComposite deserializeToCompositeToken(
            Element e) {
        org.datacontract.schemas._2004._07.abc4trust_uprove.ObjectFactory ofup = new org.datacontract.schemas._2004._07.abc4trust_uprove.ObjectFactory();
    ...
    public IssuerParametersComposite deserializeToIssuerParametersComposite(
        ...
        String string = this.getString(GQ, e);
        ipc.setGq(ofup.createIssuerParametersCompositeGq(string));
        string = this.getString(HASH_FUNCTION_OID, e);
    ...
    public PseudonymComposite deserializeToPseudonymComposite(
            Element e) {
        org.datacontract.schemas._2004._07.abc4trust_uprove.ObjectFactory ofup = new org.datacontract.schemas._2004._07.abc4trust_uprove.ObjectFactory();
