                ...
                try{
                    ...
                    if("UProveCredentialAndPseudonym".equals(elementName)){
                    	...
                    	int revocationindex = -1;
                   		...
                   		if(!committedValues.contains(revocationindex) && revocationindex != -1) committedValues.add(revocationindex);
                   		Map<Integer, Integer> indexToIndexInProof = mapIndices(committedValues);
...
//
            ...
            //	System.out.println(XmlUtils.toXml(of.createPresentationTokenWithCommitments(idemixToken), false)+"\n\n");
                ...
                if(idemixToken.getCryptoEvidence().getAny().size() == 0) verifies = true;
                else verifies = this.idemixEngine.verifyTokenWithCommitments(idemixToken);
    ...
    private Map<Integer, Integer> mapIndices(Set<Integer> inspectableAttributes) {
    	...
    	for(int i = 0; i< count; i++){
    		int key = getSmallest(inspectableAttributes);
    		ret.put(key, i);
