...
public class InspectorPrivateKeySerializer {
  ...
  public byte[] serializeInspectorPrivateKey(SecretKey secretKey) {
    ...
    try {
      ...
      ser.write(serializeBigInteger(ibmVEPrivateKey.getOrderN()));
      ser.write(serializeBigInteger(ibmVEPrivateKey.getX1()));
      ser.write(serializeBigInteger(ibmVEPrivateKey.getX2()));
      ser.write(serializeBigInteger(ibmVEPrivateKey.getX3()));
      ser.close();
    ...
    try {
      ...
      switch (data[0]) {
        case 2: {
          ...
          URI thePublicKeyLocation = URI.create(new String(baos.toByteArray()));
          BigInteger _orderN = unSerializeBigInteger(buf);
          BigInteger _x1 = unSerializeBigInteger(buf);
          BigInteger _x2 = unSerializeBigInteger(buf);
          BigInteger _x3 = unSerializeBigInteger(buf);
