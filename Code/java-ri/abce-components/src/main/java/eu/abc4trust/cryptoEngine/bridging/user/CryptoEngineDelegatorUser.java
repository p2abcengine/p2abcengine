...
public class CryptoEngineDelegatorUser implements CryptoEngineUser{
    ...
    @Inject
    public CryptoEngineDelegatorUser(IdemixCryptoEngineUserImpl idemix, UProveCryptoEngineUserImpl uprove, KeyManager keyManager, CredentialManager credManager) {
    ...
    public PresentationToken createPresentationToken(
        ...
        for(CredentialInToken cit: ptd.getCredential()){
            ...
            for(AttributeInToken ait: cit.getDisclosedAttribute()){
                ...
                if(ait.getInspectorPublicKeyUID()!= null){
                	...
                	for(CommittedAttribute ca: citwc.getCommittedAttribute()){
                		if(ca.getAttributeType().equals(ait.getAttributeType())) alreadyOnList = true;
                	}
                	if(!alreadyOnList){
                		CommittedAttribute ca = of.createCommittedAttribute();
            ...
            if(credSpec.isRevocable()){
            	boolean alreadyOnList = false;
            	for(CommittedAttribute ca: citwc.getCommittedAttribute()){
            		if(ca.getAttributeType().toString().equals("http://abc4trust.eu/wp2/abcschemav1.0/revocationhandle")) alreadyOnList = true;
            	}
            	if(!alreadyOnList){
            		CommittedAttribute ca = of.createCommittedAttribute();
    ...
    public IssuanceToken createIssuanceToken(IssuanceTokenDescription itd, List<URI> creduids,
        ...
        try{
            if((this.keyManager.getIssuerParameters(itd.getCredentialTemplate().getIssuerParametersUID())).getAlgorithmID().equals(new URI("urn:abc4trust:1.0:algorithm:uprove"))){
    ...
    public Credential updateNonRevocationEvidence(Credential cred,
    	...
    	URI algorithm = null;
    	try{
    		algorithm = keyManager.getIssuerParameters(issParams).getAlgorithmID();
    	}catch(KeyManagerException e){
    		throw new RuntimeException(e);
    	}
    	if(algorithm.equals(CryptoUriUtil.getIdemixMechanism()))  return this.idemixEngine.updateNonRevocationEvidence(cred, raparsuid, revokedatts);
        return uproveEngine.updateNonRevocationEvidence(cred, raparsuid, revokedatts);
    }
    ...
    public Credential updateNonRevocationEvidence(Credential cred,
    	...
    	if(algorithm.equals(CryptoUriUtil.getIdemixMechanism())) return this.idemixEngine.updateNonRevocationEvidence(cred, raparsuid, revokedatts, revinfouid);
        return uproveEngine.updateNonRevocationEvidence(cred, raparsuid, revokedatts, revinfouid);
    }
