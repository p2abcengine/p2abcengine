...
public class UserHelperTest {
    ...
    private void initHelper(CryptoEngine cryptoEngine, CryptoEngine userRunEngine, String user) throws Exception {
        ...
        UserHelper.initInstance(cryptoEngine, /*systemParamsResource, */issuerParamsResourceList, fileStoragePrefix, credSpecResourceList, inspectorPublicKeyResourceList);
        
        Secret secret;
        if(userRunEngine == CryptoEngine.IDEMIX) {
            System.out.println("- client engine is IDEMIX - load secret !");
            InputStream is = AbstractHelper.getInputStream(
        ...
        if(userRunEngine == CryptoEngine.IDEMIX) {
            ...
            String soderhamnPseudonymValue_Idemix_BigIntegerString = "";
            BigInteger soderhamnPseudonymValue = new BigInteger(soderhamnPseudonymValue_Idemix_BigIntegerString);
            ...
            BigInteger patrasPseudonymValue = new BigInteger(patrasPseudonymValue_Idemix_BigIntegerString);
    ...
    public void test_verifyWithJSonCallback() throws Exception {
        ...
        PresentationPolicyAlternatives ppa = (PresentationPolicyAlternatives) XmlUtils.getObjectFromXML(UserHelperTest.class.getResourceAsStream("/eu/abc4trust/sampleXml/presentationPolicies/presentationPolicySimpleIdentitycard.xml"), true);

        JSonIdentitySelection jsonIDSelect = new JSonIdentitySelection();
        PresentationToken pt = helper.getEngine().createPresentationToken(ppa,
                jsonIDSelect);
    ...
    public void getPseudonymValue(PresentationToken pt, String scope, BigInteger expectedValue) throws Exception {
    ...
    public void test_getPseudonymValue_idemix() throws Exception {
        this.initHelper(CryptoEngine.IDEMIX, CryptoEngine.IDEMIX, "alice_idemix");
        InputStream inputStream = UserHelperTest.class.getResourceAsStream("/eu/abc4trust/sampleXml/patras/presentationPolicyPatrasUniversityLogin.xml");
        @SuppressWarnings("unused")
		PresentationPolicyAlternatives ppa = (PresentationPolicyAlternatives) XmlUtils.getObjectFromXML(inputStream, true);
