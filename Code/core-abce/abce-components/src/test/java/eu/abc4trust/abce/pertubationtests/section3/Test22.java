...
public class Test22 {
    ...
    private static final Logger logger = java.util.logging.Logger.getLogger("Section 3.2.2 log");
...
//	private Injector inspectorInjector = null;
	...
	public static void setupLogger() throws SecurityException, IOException {
		Handler fh = new FileHandler("Section-3.2.2.log");
...
//		inspectorInjector.getInstance(KeyManager.class).storeSystemParameters(syspars);
  ...
  //              idcardCredSpec.getSpecificationUID(), idcardCredSpec);
	...
	public void runTestDublicateCredSpecUid() throws Exception{
    	try{
    		logger.info("Running test with dublicate Credspec uid");    		
	...
	public void runTestNonexistingCarryOver() throws Exception{
    	try{
    		...
    		runTest(idcardIssuancePolicy.getCredentialTemplate(), null);
            ...
            runTest(temp, presentationPolicyAlternatives.getPresentationPolicy().get(0));
	...
	private void runTest(CredentialTemplate credTemp, PresentationPolicy pp) throws Exception{
        ...
        URI idcardIssuancePolicyUid = credTemp.getIssuerParametersUID();
        ...
        try{
        	...
        	List<Attribute> issuerAtts = this.populateIssuerAttributes(
                    att, idcardCredSpec);
//Test specific
        	IssuancePolicy origidcardIssuancePolicy = (IssuancePolicy) XmlUtils
                    .getObjectFromXML(
                            this.getClass().getResourceAsStream(
                                    ISSUANCE_POLICY_ID_CARD), true);
        	
            IssuanceMessageAndBoolean issuerIm = issuerEngine.initIssuanceProtocol(
                    origidcardIssuancePolicy, issuerAtts);
            assertFalse(issuerIm.isLastMessage());
            
            List<Object> modifiedList = new LinkedList<Object>();
            for(Object o: issuerIm.getIssuanceMessage().getContent()){
            	if(((JAXBElement)o).getValue() instanceof IssuancePolicy){
            		IssuancePolicy ip = (IssuancePolicy)((JAXBElement)o).getValue();
            		ip.setCredentialTemplate(credTemp);
            		if(pp != null){
            			pp.setMessage(ip.getPresentationPolicy().getMessage());
            ...
            issuerIm.getIssuanceMessage().getContent().clear();
            issuerIm.getIssuanceMessage().getContent().addAll(modifiedList);
 //-test specific            
