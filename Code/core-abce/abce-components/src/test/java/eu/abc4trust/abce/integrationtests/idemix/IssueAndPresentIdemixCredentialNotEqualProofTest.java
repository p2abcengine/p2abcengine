 ...
 * @author mdu
 */
public class IssueAndPresentIdemixCredentialNotEqualProofTest {
    ...
    public void issueCredentialTest() throws Exception{
        ...
        try {
            ...
            if (DEBUG) {System.out.println(XmlUtils.toNormalizedXML(of.createCredentialSpecification(credSpec)));}
        ...
        if (DEBUG) {String cds = XmlUtils.toXml(of.createCredentialDescription(cd));
        ...
        PresentationPolicyAlternatives ppa =
                (PresentationPolicyAlternatives) XmlUtils.getObjectFromXML(this.getClass().getResourceAsStream(
                        "/eu/abc4trust/sampleXml/idemixIntegration/presentationPolicyCreditCardIdmxTestNotEqual.xml"), true);
        ...
        if (DEBUG) {String pts = XmlUtils.toXml(of.createPresentationToken(presentationToken));
        ...
        if (DEBUG) {String ptds = XmlUtils.toXml(of.createPresentationTokenDescription(ptd));
