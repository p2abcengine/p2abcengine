...
public class CredentialManagerImpl implements CredentialManager {
    ...
    private URI getSmartcardUri(){
        ...
        for(URI uri : scs.keySet()){
            BasicSmartcard basicSmartcard = scs.get(uri);
            if(!(basicSmartcard instanceof SecretBasedSmartcard)){
                return uri;
            }
        }
        return null;
        ...
        try {
            ...
            for(URI uri : this.cardStorage.getSmartcards().keySet()){
                ...
                if(sc instanceof Smartcard){
                    ...
                    Credential cred = sc.getCredential(this.cardStorage.getPin(uri), creduid, this.credentialSerializer);
                    if(cred != null){
        ...
        try {
            ...
            if((cardUid != null) &&  (bsc != null) && !(bsc instanceof SecretBasedSmartcard)){
                if(!cred.getCredentialDescription().isRevokedByIssuer()) {
                    ...
                    TimingsLogger.logTiming("Smartcard.storeCredential(" + cred.getCredentialDescription().getCredentialSpecificationUID() + ")", true);
                    SmartcardStatusCode status = sc.storeCredential(this.cardStorage.getPin(cardUid), credUid, cred, this.credentialSerializer);
                    TimingsLogger.logTiming("Smartcard.storeCredential(" + cred.getCredentialDescription().getCredentialSpecificationUID() + ")", false);
    ...
    public void storePseudonym(PseudonymWithMetadata pwm)
        ...
        try {
            ...
            if(scUri == null){
                ...
                SmartcardStatusCode code = sc.storePseudonym(this.cardStorage.getPin(scUri), pseudonymUri, pwm, this.pseudonymSerializer);
        ...
        try {
            for (URI credUid : this.listCredentials()) {
                ...
                CredentialSpecification credSpec = this.keyManager
                        .getCredentialSpecification(credDesc
                                .getCredentialSpecificationUID());
            ...
            if(scUri == null){
                ...
                for(URI uri: blobs.keySet()){
                    ...
                    if(uriString.startsWith(PSEUDONYM_PREFIX)){
                        ...
                        PseudonymWithMetadata pwm = sc.getPseudonym(this.cardStorage.getPin(scUri), pseudonymUri, this.pseudonymSerializer);
                        Pseudonym pseudonym = pwm.getPseudonym();
                        if (pseudonym.getScope().equals(scope)
                                && !(onlyExclusive && !pseudonym.isExclusive())) {
                            ls.add(pwm);
    ...
    public PseudonymWithMetadata getPseudonymWithMetadata(Pseudonym pseudonym)
        ...
        return this.getPseudonym(pseudonymUid);
        /*
        try {
            URI pseudonymUid = pseudonym.getPseudonymUID();
            URI scUri = this.getSmartcardUri();
