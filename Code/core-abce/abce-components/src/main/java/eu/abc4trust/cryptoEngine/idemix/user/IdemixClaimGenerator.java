...
public class IdemixClaimGenerator {
    ...
    public VerifiableClaim generatePresentationEvidence(final Claim claim,
        ...
        long pr_evidence_start = System.nanoTime();
        IdemixClaim ic = (IdemixClaim) claim;

        TimingsLogger.logTiming("IdemixClaimGenerator.generatePresentationEvidence", true);
        ...
        if(ic.getInspectableAttributes().size() > 0){
            ...
            for(MyAttributeReference mar: ic.getInspectableAttributes()){
                ...
                TimingsLogger.logTiming("new VerifiableEncryptionOpening", true);
                VerifiableEncryptionOpening pEnc1 = new VerifiableEncryptionOpening(
                        message, r1, vepkId, label);
                TimingsLogger.logTiming("new VerifiableEncryptionOpening", false);

                TimingsLogger.logTiming("VerifiableEncryptionOpening.getEncryption", true);
                VerifiableEncryption vEnc1 = pEnc1.getEncryption();
                TimingsLogger.logTiming("VerifiableEncryptionOpening.getEncryption", false);
        ...
        if (this.DEBUG) {System.out.println("SPEC_PROVER\n" + proofSpec.toStringPretty());}
        ic.setEvidence(proof);

        long pr_evidence_stop = System.nanoTime();

        if (this.DEBUG) {System.out.println(">> timing create presentation claim idemix crypto engine: "+ ((pr_evidence_stop - pr_evidence_start)/1000000));}

        TimingsLogger.logTiming("IdemixClaimGenerator.generatePresentationEvidence", false);
        return ic;
    }
