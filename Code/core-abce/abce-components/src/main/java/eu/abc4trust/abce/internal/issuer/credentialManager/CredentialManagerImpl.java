    ...
    public SecretKey getIssuerSecretKey(URI issuerParamsUid)
        ...
        if (issuerParamsUid == null) {
            throw new CredentialManagerException(
                    "Issuer parameters UID is null");
        }
        ObjectInputStream objectInput = null;
        ByteArrayInputStream byteArrayInputStream = null;
        try {
            byte[] tokenBytes = this.storage
                    .getIssuerSecretKey(issuerParamsUid);
            if (tokenBytes == null) {
                throw new IssuerSecretKeyNotInStorageException(
                        "Issuer secret key with UID: \"" + issuerParamsUid
                        + "\" is not in storage");
            }
            byteArrayInputStream = new ByteArrayInputStream(tokenBytes);
            objectInput = new ObjectInputStream(byteArrayInputStream);
            SecretKey issuerSecretKey = (SecretKey) objectInput
                    .readObject();
