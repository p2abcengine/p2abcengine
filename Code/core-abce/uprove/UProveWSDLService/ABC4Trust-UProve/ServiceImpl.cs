...
namespace abc4trust_uprove
{
  ...
  public class Service1 : IService1
  {
    ...
    private Service1() { }
    private static readonly Log cOut = Logger.Instance.getLog(LoggerDefine.OUT_CONSOLE);
    private static readonly Log dOut = Logger.Instance.getLog(LoggerDefine.DEBUG_CONSOLE);
    ...
    private static readonly ConcurrentDictionary<string, SessionData> sessionDB = new ConcurrentDictionary<string, SessionData>();
    ...
    private const int DevicePseudonymIndex = 254;
    ...
    private static IssuerKeyAndParametersComposite UsePreComputeParameterSet(string uniqueIdentifier, byte[] attributeEncoding, string hash, string sessionID)
    {
      IssuerSetupParameters isp = new IssuerSetupParameters
      {
        GroupConstruction = GroupConstruction.Subgroup,
        ParameterSet = sessionDB[sessionID].parameterSet
      };
      ...
      IssuerKeyAndParametersComposite ikpc = new IssuerKeyAndParametersComposite
      {
        IssuerParameters = ConvertUtils.convertIssuerParameters(ikap.IssuerParameters),
        PrivateKey = ikap.PrivateKey.ToByteArray()
      };
